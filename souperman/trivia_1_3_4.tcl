...
# synced with v 1.3.4 O.27apr'16                                     #
######################################################################
...
## pjo set tghtmlfile "scripts/trivia.html"
...
## pjo ##  set tgchan "#dutch"  (2018, 2025:#balkon)
set tgchan "#<yourchan>"
...
#  Show Vraags in all CAPS (1) or not (0)?
set tgcapsVraag 0
...
# no colour
set tgcolourstart "01"               ;#Game has started.
set tgcolourstop "01"                ;#Game has stopped.
set tgcolourskip "01"           ;#Vraag has been skipped.
set tgcolourerr "01"            ;#How to report errors.
set tgcolourmiss "01"           ;#Nobody answered the Vraag.
set tgcolourqhead "01"          ;#Vraag heading.
set tgcolourqbody "01"               ;#Vraag itself.
set tgcolourhint "01"           ;#Hint.
set tgcolourstrk "01"        ;#Person is on a winning streak.
...
#	if {![info exists tgstreak]} {set tgstreak 0}
...
proc tgnextq {} {
 ...
 global tgcapsVraag tgcapsanswer
	...
	set tgcurrentVraag ""
	set tgcurrentanswer ""
	set ::tgmaxhintcurrent $::tgmaxhint
	while {$tgcurrentVraag == ""} {
		...
		set tgcurrentanswer [string trim [lindex [split $tgVraagselected $tgqdbsep] $tgqdbVraagfirst]]
		if {$tgcapsanswer==1} {
...
#	set tgtimestart [clock milliseconds]
#	if {![info exists tgtimestart]} {set tgtimestart [clock milliseconds]}
#        set ::tgtimestart $tgtimestart
...
proc tgcorrectanswer {nick} {
	...
	set _timetoanswer ""
	if {$tgtimeanswer==1} {
		set _timetoanswer [expr [expr [clock milliseconds]-$tgtimestart]/1000]
	}
	set _msg "[tgcolmisc1][lindex $tgcongrats [rand [llength $tgcongrats]]] [tgcolmisc2]$nick[tgcolmisc1]! Het antwoord was [tgcolmisc2]$tgcurrentanswer[tgcolmisc1].[expr $tgtimeanswer==1?" You got it in [tgcolmisc2]$_timetoanswer[tgcolmisc1] seconds.":""]"
...
proc tgshowscores {nick} {
	...
	if {$tgshowallscores} {
		...
		tggamemsg "[tgcolmisc2]$nick [tgcolmisc1]heeft nu [tgcolmisc2]$tgscoresbyname([strlwr $nick]) [tgcolmisc1][expr $tgscoresbyname([strlwr $nick])==1?"point":"points"] and is ranked [tgcolmisc2][ordnumber $tgranksbyname([strlwr $nick])] [tgcolmisc1]of [tgcolmisc2]$tgscorestotal[tgcolmisc1]$_tgt"
	}
}
...
proc tgscorelookup {nick host hand text} {
	...
	if {![info exists tgscoresbyname([strlwr $text])]} {
		...
		[tgpriv] $nick "[tgbold]$_who [tgcolmisc2]$tgscoresbyname([strlwr $text])[tgcolmisc1] punten, gerangschikt [tgcolmisc2][ordnumber $tgranksbyname([strlwr $text])] [tgcolmisc1]of [tgcolmisc2]$tgscorestotal[tgcolmisc1]."
	}
}
...
proc tgtargetlookup {nick host hand text} {
	...
	if {![info exists tgscoresbyname([strlwr $nick])]} {
		...
		[tgpriv] $nick "[tgbold][tgcolmisc1]Je bent op [tgcolmisc2]$tgscoresbyname([strlwr $nick])[tgcolmisc1]. Your next target is [tgcolmisc2]$tgrealnames($tgranksbynum([expr $tgranksbyname([strlwr $nick])-1])) [tgcolmisc1]with [tgcolmisc2]$tgscoresbyname($tgranksbynum([expr $tgranksbyname([strlwr $nick])-1]))[tgcolmisc1], ranked [tgcolmisc2][ordnumber [expr $tgranksbyname([strlwr $nick])-1]] [tgcolmisc1]of [tgcolmisc2]$tgscorestotal[tgcolmisc1]."
	}
}
...
proc tgerror {nick host hand text} {
	...
	if {$text==""||![string is int [lindex [split $text] 0]]} {
		[tgpriv] $nick "[tgbold][tgcolmisc1]Specificeer het vraagnummer."
		return
	}
	tgreadqdb
	set _qnum [lindex [split $text] 0]
	if {$_qnum>$tgVraagstotal} {
		[tgpriv] $nick "[tgbold][tgcolmisc1]Vraag bestaat niet."
		return
	}
	...
	if {$_desc==""} { set _desc "Geen verdere info voor deze fout." }
	if {$tgerrmethod==1} {
	...
	puts $_file "Antwoord:\t\t$_qans"
	puts $_file "Commentaar:\t$_desc"
	puts $_file "------------------------------"
	close $_file
	if {$tgerrmethod==1} {
...
proc tghtml {} {
	...
	if {![onchan $botnick $tgchan]} {
		...
		puts $_file "  <p>Trivia game is currently <b>[expr $tgplaying==1?"on":"off"]</b>. There are <b>$tgVraagstotal</b> vragen in de database."
...
putlog "======================================================="
putlog "* trivia.tcl $tgver ($tgrel) by Souperous loaded."
